...
def _parse_band(text):
    """Parses 'lo,hi' into a (lo, hi) float tuple, for the --band argument."""
    try:
        lo_str, hi_str = text.split(",")
        return (float(lo_str), float(hi_str))
    except ValueError:
        raise argparse.ArgumentTypeError(
            f"Invalid --band value '{text}': expected 'lo,hi', e.g. 54.5,55.5"
        )
...
def _common_args(p):
    ...
    p.add_argument("--p-planet", type=float, action="append", required=True,
                    dest="p_planets",
                    help="Planet period (days). Repeat for several planets, "
                         "e.g. --p-planet 19.25 --p-planet 8.6")
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="spot_wave", description="Wavelet filtering pipeline")
    sub = parser.add_subparsers(dest="mode", required=True)

    p_single = sub.add_parser("single", help="Simple filter: one w0 range, one or more bands")
    _common_args(p_single)
    p_single.add_argument("--band", type=_parse_band, action="append", required=True,
                           dest="bands",
                           help="Period band to filter, as 'lo,hi' in days. Repeat "
                                "for several bands removed together at each w0, "
                                "e.g. band 54.5,55.5, band 27.0,28.0")
    ...
    p_planets = args.p_planets  # list[float], one or more planets

    if args.mode == "single":
        w0_grid = make_w0_grid(args.w0_min, args.w0_max, args.w0_step)
        best = single_filter_sweep(
            t, rv, rv_err, w0_grid, bands=args.bands,
            p_rot=args.p_rot, p_rot_half=p_rot_half, p_planets=p_planets,
        ...
        best = run_double_filter_sweep(
            ...
            planet_periods=p_planets, order1=order1, order2=order2,
            permin=args.permin, permax=args.permax, verbose=verbose,
        )
        out_file = os.path.join(
            args.out_dir,
